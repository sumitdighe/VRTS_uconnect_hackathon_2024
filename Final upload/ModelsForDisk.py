...
data = pd.read_csv('merged_disk_write_bytes.csv')
data = data.dropna()
...
isolation_forest_model = IsolationForest(n_estimators=120, contamination=0.1, max_samples=0.5)
...
def get_disk_metrics():
    return {
        ...
        'value': ps.disk_io_counters().write_bytes
...
while True:
    ...
    real_cpu_data = pd.read_csv('realdataset4.csv')
    real_cpu_data['timestamp'] = pd.to_datetime(data['timestamp'])
    real_cpu_data['hour'] = real_cpu_data['timestamp'].dt.hour

    df2 = pd.DataFrame(real_cpu_data,columns=['hour', 'value'])
    ...
    combined_prediction = np.multiply(isolation_forest_prediction, svm_prediction)

    # Identify combined outliers
    combined_outliers = np.where(combined_prediction == -1)[0]

    # Plot outliers identified by both models
    plt.scatter(df2['hour'][combined_outliers], df2['value'][combined_outliers], color='purple', label='Combined Outliers')
    plt.scatter(df2['hour'][isolation_forest_prediction == -1], df2['value'][isolation_forest_prediction == -1],
                color='red', label='Isolation Forest Outliers')
    plt.scatter(df2['hour'][svm_prediction == -1], df2['value'][svm_prediction == -1],
    ...
    print("Disk anomalies: ",combined_prediction)
