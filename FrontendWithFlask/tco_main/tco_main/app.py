...
def collect_server_workload():
    ...
    workload_type = str(request.form['workload_type'])
...
def generate_report(cost_details, environment):
    ...
    filename = f"cost_breakdown_{environment}_{uuid.uuid4()}.png"
    filepath = os.path.join('static', 'images', filename)
    plt.savefig(filepath)
    plt.close(fig)  # Close the plot to free up memory
    return filename  # Return the path where the file is saved
...
def index():
    if request.method == 'POST':
        ...
        for provider in cloud_providers:
            ...
            cloud_graph_filename = generate_report(cloud_details, provider)
            cloud_graphs.append(cloud_graph_filename)
        ...
        on_premises_graph_filename = generate_report(on_premises_details, 'On-Premises')
        cloud_graphs.append(on_premises_graph_filename)
        ...
        return render_template('result.html', tco_result="Your TCO calculation is complete!",cost_details=cost_projections,graph_filenames=cloud_graphs,predicted_deployment=predicted_deployment)
